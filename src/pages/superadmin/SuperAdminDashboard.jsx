...
const PLANS = [
  { id:'free',       label:'Free',       color:'#888',    bg:'#F1EFE8', members:10,  depts:1 },
  { id:'starter',    label:'Starter',    color:'#0C447C', bg:'#E6F1FB', members:50,  depts:5 },
  { id:'growth',     label:'Growth',     color:'#27500A', bg:'#EAF3DE', members:100, depts:10 },
  { id:'enterprise', label:'Enterprise', color:'#3C3489', bg:'#EEEDFE', members:999, depts:999 },
]
...
const SuperAdminDashboard = () => {
  ...
  const [planModal,  setPlanModal]  = useState(null) // company to edit plan
  ...
  const assignPlan = async (companyId, planId) => {
    await updateDoc(doc(db, 'companies', companyId), { plan: planId })
    setNotify({ msg:`Plan updated to ${PLANS.find(p=>p.id===planId)?.label}.`, type:'ok' })
    setPlanModal(null)
    loadCompanies()
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f0f1a', color:'#e0e0e0', fontFamily:'system-ui,sans-serif' }}>
      ...
      <div style={{ padding:'24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:'12px', marginBottom:'24px' }}>
          {PLANS.map(p => (
            <div key={p.id} style={{ background:'#1a1a2e', border:'1px solid #2a2a4a', borderRadius:'10px', padding:'14px' }}>
              <div style={{ fontSize:'11px', color:'#666', marginBottom:'4px' }}>{p.label} plan</div>
              <div style={{ fontSize:'22px', fontWeight:600, color:p.color }}>
                {companies.filter(c => (c.plan||'free') === p.id).length}
              </div>
              <div style={{ fontSize:'11px', color:'#555' }}>companies</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:'flex', gap:'10px', marginBottom:'16px' }}>
          <input style={{ flex:1, padding:'9px 14px', fontSize:'13px', background:'#1a1a2e', border:'1px solid #2a2a4a', borderRadius:'8px', color:'#e0e0e0', fontFamily:'inherit' }} placeholder="Search company name or ID..." value={search} onChange={e => setSearch(e.target.value)} />
          ...
          {loading ? (
            ...
            return (
              <div key={c.id} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'14px 16px', borderBottom: i < filtered.length-1 ? '1px solid #1e1e3a' : 'none', flexWrap:'wrap' }}>
                ...
                <button
                  onClick={() => setPlanModal(c)}
                  style={{ fontSize:'11px', padding:'4px 12px', borderRadius:'20px', fontWeight:600, background:plan.bg, color:plan.color, border:'none', cursor:'pointer', fontFamily:'inherit' }}
                >
                  {plan.label} ▾
                </button>
      ...
      {planModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }}>
          <div style={{ background:'#1a1a2e', border:'1px solid #2a2a4a', borderRadius:'16px', padding:'28px', width:'100%', maxWidth:'400px', margin:'20px' }}>
            <div style={{ fontSize:'15px', fontWeight:600, color:'#fff', marginBottom:'6px' }}>Assign plan</div>
            <div style={{ fontSize:'12px', color:'#666', marginBottom:'20px' }}>{planModal.name}</div>
            <div style={{ display:'flex', flexDirection:'column', gap:'10px', marginBottom:'20px' }}>
              {PLANS.map(p => (
                <button
                  key={p.id}
                  onClick={() => assignPlan(planModal.id, p.id)}
                  style={{
                    display:'flex', alignItems:'center', justifyContent:'space-between',
                    padding:'12px 16px', borderRadius:'10px', cursor:'pointer', fontFamily:'inherit',
                    background: (planModal.plan||'free') === p.id ? '#2a2a4a' : 'transparent',
                    border: `1px solid ${(planModal.plan||'free') === p.id ? '#378ADD' : '#2a2a4a'}`,
                    color:'#fff',
                  }}
                >
                  <div style={{ textAlign:'left' }}>
                    <div style={{ fontSize:'14px', fontWeight:500 }}>{p.label}</div>
                    <div style={{ fontSize:'11px', color:'#666', marginTop:'2px' }}>Up to {p.members === 999 ? 'unlimited' : p.members} members · {p.depts === 999 ? 'unlimited' : p.depts} depts</div>
                  </div>
                  {(planModal.plan||'free') === p.id && <span style={{ fontSize:'12px', color:'#378ADD' }}>Current</span>}
                </button>
              ))}
            </div>
            <button onClick={() => setPlanModal(null)} style={{ width:'100%', padding:'10px', fontSize:'13px', background:'transparent', border:'1px solid #2a2a4a', borderRadius:'8px', color:'#aaa', cursor:'pointer', fontFamily:'inherit' }}>
              Cancel
            </button>
          </div>
        </div>
